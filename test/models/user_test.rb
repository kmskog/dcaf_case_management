...
class UserTest < ActiveSupport::TestCase
  ...
  describe 'call list methods' do
    before do
      ...
      @patient_2 = create :patient, city: @city
      @md_patient = create :patient, city: @city2
    ...
    it 'should return recently_called_patients accurately' do
      assert_equal 0, @user.recently_called_patients(@city).count
      ...
      assert_equal 2, @user.recently_called_patients(@city).count
      assert_equal 1, @user.recently_called_patients(@city2).count
    end

    it 'should return call_list_patients accurately' do
      assert_equal 2, @user.call_list_patients(@city).count
      assert_equal 1, @user.call_list_patients(@city2).count

      with_versioning(@user) do
        @patient.calls.create attributes_for(:call)
      end
      assert_equal 1, @user.call_list_patients(@city).count
    ...
    it 'should clean calls when patient has been reached' do
      ...
      assert_equal 1, @user.recently_called_patients(@city).count
      assert_difference '@user.recently_called_patients(@city).count', -1 do
    ...
    it 'should clear call list when someone invokes the cleanout' do
      assert_difference '@user.call_list_entries.count', -2 do
        assert_no_difference '@user.call_list_entries.where(city: @city2).count' do
          assert_no_difference 'Patient.count' do
            @user.clear_call_list @city
          end
        end
      end
    end
  ...
  describe 'patient methods' do
    before do
      @city = create :city
      @city2 = create :city
      @patient = create :patient, city: @city2
      @patient_2 = create :patient, city: @city
      @patient_3 = create :patient, city: @city
    end
    ...
    describe 'reorder call list' do
      before do
        ...
        @user.reorder_call_list new_order, @city
      end

      it 'should let you reorder a call list' do
        assert_equal @patient_3, @user.call_list_patients(@city).first
        assert_equal @patient_2, @user.call_list_patients(@city)[1]
      end

      it 'should always add new patients to the front of the call order' do
        @patient_4 = create :patient, city: @city
        @user.add_patient @patient_4

        assert @user.call_list_patients(@city).include? @patient_4
        assert_equal @user.call_list_patients(@city).map { |x| x.id.to_s }.index(@patient_4.id.to_s), 0
      end
    end
  end
  ...
  describe 'relationships' do
    ...
    it 'should have any belong to many patients' do
      ...
      assert_equal @user.call_list_entries.pluck(:patient_id, :city_id, :order_key),
                   @user_2.call_list_entries.pluck(:patient_id, :city_id, :order_key)
    end
  end
