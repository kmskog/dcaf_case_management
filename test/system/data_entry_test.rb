...
class DataEntryTest < ApplicationSystemTestCase
  ...
  describe 'entering a new patient' do
    before do
      # fill out the form
      select @city.name, from: 'patient_city_id'
      fill_in 'Intake Date', with: 2.days.ago.strftime('%m/%d/%Y')
    ...
    it 'should log a new patient ready for further editing: procedure' do
  ...
  describe 'entering a new backdated patient' do
    before do
      # fill out the form
      select @city.name, from: 'patient_city_id'
      fill_in 'Intake Date', with: 90.days.ago.strftime('%m/%d/%Y')
